...
public class VideoJuego {
    public static void main(String[] args) {
        ...
        // Aqui hago unas copias de los ejercitos para el ranking, asi evito modificar los originales
...
/*
public static void batalla(Soldado s1, Soldado s2, ArrayList<Soldado> ejercitoEnemigo, 
                                                                        ArrayList<ArrayList<Soldado>> tablero) {
    if (s1.getPuntosVida() > s2.getPuntosVida()) {
        System.out.println(s1.getNombre() + " ha derrotado a " + s2.getNombre() + "\n");
        ejercitoEnemigo.remove(s2);
    ...
    } else if (s1.getPuntosVida() == s2.getPuntosVida()){
        System.out.println("¡Empate! Ambos soldados han muerto.\n");
        ejercitoEnemigo.remove(s2);
        ejercitoEnemigo.remove(s1);

        // Quita a los soldados derrotados
        tablero.get(s1.getFila() - 1).set(s1.getColumna() - 'A', null);
        tablero.get(s2.getFila() - 1).set(s2.getColumna() - 'A', null);

    } else {
        System.out.println(s2.getNombre() + " ha derrotado a " + s1.getNombre() + "\n");
        ejercitoEnemigo.remove(s1);

        // Saca al soldado derrotado
        tablero.get(s1.getFila() - 1).set(s1.getColumna() - 'A', s2);

        // Actualiza la posición
        tablero.get(s2.getFila() - 1).set(s2.getColumna() - 'A', null);
        s2.setFila(s1.getFila());
        s2.setColumna(s1.getColumna());
    }
}
 */